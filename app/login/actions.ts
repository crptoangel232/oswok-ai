...
export async function login(formData: FormData) {
  ...
  if (!email || !email.includes('@') || password.length < 8) {
    errorRedirect('Enter a valid email and a password of at least 8 characters.', requestedRole)
  }
  ...
  if (profile.onboarding_completed && profile.role !== requestedRole) {
    errorRedirect(`This account is registered as a ${profile.role}. Choose ${profile.role} to continue.`, requestedRole)
  }
