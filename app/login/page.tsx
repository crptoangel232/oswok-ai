    ...
    <main className="min-h-screen bg-slate-950 px-5 py-12 text-white">
      <div className="mx-auto max-w-md">
        ...
        <form className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5 shadow-2xl">
          ...
          {isEmployer ? (
            <div className="rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-4 text-sm text-cyan-100">You are entering Oswok as an <strong>employer</strong>. After onboarding, you can post a job and manage applications.</div>
          ) : (
            <div className="rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-4 text-sm text-cyan-100">You are entering Oswok as a <strong>worker</strong>. After onboarding, you can find jobs and apply.</div>
          )}
        ...
        <p className="mt-5 text-xs leading-5 text-slate-500">{isEmployer ? 'Employers publish jobs. Workers see published jobs and can apply.' : 'Workers see published jobs and can apply from the job page.'}</p>
      </div>
    </main>
  )
}
