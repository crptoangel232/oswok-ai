...
export default async function EmploymentPage({ searchParams }: { searchParams: Promise<{ hired?: string; job?: string }> }) {
        ...
        <section className="mt-7 space-y-4">
          {(employments ?? []).map((employment) => {
            ...
            return <article key={employment.employment_id} className="rounded-2xl border border-white/10 bg-white/5 p-6">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-cyan-300">{profile.role === 'employer' ? 'Worker hired' : 'Employer'}</p><h2 className="mt-2 text-xl font-bold">{employment.job_title}</h2><p className="mt-1 text-sm text-slate-400">{otherParty}</p></div><span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-semibold capitalize text-emerald-200">{employment.status}</span></div>
              <p className="mt-5 text-sm text-slate-400">Started {new Date(employment.started_at).toLocaleDateString('en-GB')}</p>
              <Link href={`/jobs/${employment.job_id}`} className="mt-4 inline-flex text-sm font-semibold text-cyan-300">View job →</Link>
            </article>
          })}
