...
export async function transitionEmployment(formData: FormData) {
  ...
  const { error } = await supabase.rpc('transition_my_employment', { target_employment_id: employmentId, new_status: status })
...
export async function confirmPayment(formData: FormData) {
  ...
  const supabase = await requireUser()
  const { error } = await supabase.rpc('confirm_employment_payment', { target_employment_id: employmentId })
  if (error) redirect(`/employment/${employmentId}?error=${encodeURIComponent(error.message)}`)
  revalidatePath('/employment'); revalidatePath(`/employment/${employmentId}`); revalidatePath('/dashboard')
...
export async function submitReview(formData: FormData) {
  ...
  const supabase = await requireUser()
  const { error } = await supabase.rpc('review_my_employment', { target_employment_id: employmentId, new_rating: rating, new_comment: comment || undefined })
  if (error) redirect(`/employment/${employmentId}?error=${encodeURIComponent(error.message)}`)
