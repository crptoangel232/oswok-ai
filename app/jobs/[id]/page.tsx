...
export default async function JobPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ applied?: string; error?: string }> }) {
  ...
  const { data: application } = profile?.role === 'worker'
    ? await supabase.from('applications').select('id, status').eq('job_id', id).eq('worker_id', userId).maybeSingle()
    : { data: null }
        ...
        <article className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">{employer?.organisation_name || 'Oswok employer'} · {job.location || 'Location flexible'}</p>
            </div>
            <div className="rounded-xl bg-emerald-400/10 px-4 py-2 text-sm font-semibold text-emerald-300">{job.status}</div>
          </div>

          <div className="mt-7 border-y border-white/10 py-5">
            <p className="text-sm text-slate-400">Pay</p>
            <p className="mt-1 text-2xl font-bold">{job.pay_currency} {Number(job.pay_amount).toLocaleString()}</p>
          </div>
          ...
          {profile?.role === 'worker' && job.status === 'open' ? (
            application ? (
              <div className="mt-7 rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-5">
                <p className="font-semibold">You already applied</p>
                <p className="mt-1 text-sm text-slate-400">Application status: <span className="capitalize text-slate-200">{application.status}</span></p>
              </div>
            ) : (
              <form action={applyToJob} className="mt-7 space-y-4 rounded-xl border border-white/10 bg-slate-900/60 p-5">
                <input type="hidden" name="jobId" value={job.id} />
                <label htmlFor="coverNote" className="block text-sm font-medium">Why are you a good fit? <span className="text-slate-500">(optional)</span></label>
                <textarea id="coverNote" name="coverNote" rows={5} placeholder="Briefly tell the employer about your relevant experience." className="w-full rounded-xl border border-white/10 bg-slate-950 px-4 py-3" />
                <button type="submit" className="w-full rounded-xl bg-cyan-300 px-4 py-3 font-semibold text-slate-950">Apply for this job</button>
              </form>
            )
          ) : null}

          {isOwner ? <div className="mt-7 rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-5 text-sm text-slate-300">This is your job posting. Applicant management will be added in the next marketplace increment.</div> : null}
        </article>
      </div>
    </main>
  )
