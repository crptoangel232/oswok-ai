    ...
    <main className="min-h-screen bg-slate-950 px-5 py-10 text-white">
      <div className="mx-auto max-w-3xl">
        ...
        <article className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-7">
          ...
          {isOwner ? <div className="mt-7 rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-5"><p className="font-semibold">Manage applicants</p><p className="mt-1 text-sm text-slate-400">Review applications, read worker notes, and accept or reject candidates.</p><div className="mt-4 flex flex-wrap gap-3"><Link href={`/jobs/${id}/manage`} className="inline-flex rounded-xl bg-cyan-300 px-4 py-2 font-semibold text-slate-950">View applicants</Link><Link href={`/matches?job=${id}`} className="inline-flex rounded-xl border border-white/10 px-4 py-2 font-semibold text-cyan-200">Find recommended workers</Link></div></div> : null}
        </article>
      </div>
    </main>
  )
