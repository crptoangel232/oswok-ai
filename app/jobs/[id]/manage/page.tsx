...
import { hireWorker, transitionJobStatus, updateApplicationStatus } from '../../actions'

const nextStatuses: Record<string, string[]> = {
  draft: ['open', 'cancelled'],
  open: ['paused', 'closed', 'cancelled', 'expired', 'suspended'],
  paused: ['open', 'cancelled', 'expired', 'suspended'],
  matched: ['open', 'cancelled', 'disputed', 'suspended'],
...
type Applicant = {
  application_id:string; worker_id:string; worker_name:string|null; worker_location:string|null; worker_bio:string|null;
  verification_status:string; availability:string|null; hourly_rate:number|null; experience_years:number|null;
  status:string; cover_note:string|null; applied_at:string
}
...
export default async function ManageApplicantsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; updated?: string }> }) {
  ...
  const { data: applicants, error } = await (supabase.rpc as unknown as (name: string, args: Record<string, string>) => Promise<{ data: Applicant[] | null; error:{message:string}|null }> )('get_my_job_applicants', { target_job_id: id })
  const { data: employment } = await supabase.from('employments').select('id, worker_id, status').eq('job_id', id).maybeSingle()
    ...
    <main className="min-h-screen bg-slate-950 px-5 py-10 text-white">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-sm font-semibold text-cyan-300">EMPLOYER</p><h1 className="mt-2 text-3xl font-bold">Applicants for {job.title}</h1><p className="mt-2 text-slate-400">Review applications, shortlist candidates and hire a worker.</p></div>
        ...
        <div className="mt-7 space-y-5">
          {applicants?.length ? applicants.map((applicant) => (
            <article key={applicant.application_id} className="rounded-2xl border border-white/10 bg-white/5 p-6">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold">{applicant.worker_name || 'Worker'}</h2><p className="mt-1 text-sm text-slate-400">{applicant.worker_location || 'Location not set'} · {applicant.availability || 'Availability not set'}</p></div><span className="rounded-full bg-white/10 px-3 py-1 text-xs capitalize">{applicant.status}</span></div>
              ...
              {!employment && applicant.status !== 'rejected' ? <div className="mt-6 flex flex-wrap gap-3">
                {applicant.status !== 'shortlisted' ? <form action={updateApplicationStatus}><input type="hidden" name="applicationId" value={applicant.application_id}/><input type="hidden" name="jobId" value={id}/><input type="hidden" name="status" value="shortlisted"/><button className="rounded-xl border border-cyan-300/20 px-4 py-2 text-sm font-semibold text-cyan-200">Shortlist</button></form> : null}
                <form action={hireWorker}><input type="hidden" name="applicationId" value={applicant.application_id}/><input type="hidden" name="jobId" value={id}/><button className="rounded-xl bg-emerald-300 px-4 py-2 text-sm font-semibold text-slate-950">Hire worker</button></form>
