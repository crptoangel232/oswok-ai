...
export async function applyToJob(formData: FormData) {
  ...
  const { supabase } = await requireUser()
  const { data: applicationId, error } = await supabase.rpc('apply_to_job', { target_job_id: jobId, new_cover_note: coverNote || undefined })
  if (error || !applicationId) fail(`/jobs/${jobId}`, error?.message ?? 'Unable to apply for this job.')
  revalidatePath(`/jobs/${jobId}`); revalidatePath('/jobs'); revalidatePath('/dashboard'); revalidatePath('/applications'); redirect(`/jobs/${jobId}?applied=1`)
}
...
export async function updateApplicationStatus(formData: FormData) {
  ...
  if (!['pending', 'shortlisted', 'rejected'].includes(status)) fail(`/jobs/${jobId}/manage`, 'Invalid application status.')
  const { supabase } = await requireUser()
  const { error } = await supabase.rpc('update_my_job_application', { target_application_id: applicationId, new_status: status })
  if (error) fail(`/jobs/${jobId}/manage`, error.message)
  revalidatePath(`/jobs/${jobId}`); revalidatePath(`/jobs/${jobId}/manage`); revalidatePath('/dashboard'); revalidatePath('/applications')
...
export async function hireWorker(formData: FormData) {
  ...
  if (!applicationId || !jobId) fail('/jobs', 'Application not found.')
  const { supabase } = await requireUser()
  const { data: employmentId, error } = await supabase.rpc('hire_worker', { target_application_id: applicationId })
  if (error || !employmentId) fail(`/jobs/${jobId}/manage`, error?.message ?? 'Unable to hire this worker.')
  revalidatePath(`/jobs/${jobId}`); revalidatePath(`/jobs/${jobId}/manage`); revalidatePath('/dashboard'); revalidatePath('/applications'); revalidatePath('/employment'); revalidatePath('/matches')
  redirect(`/employment?job=${jobId}&hired=1`)
}
...
export async function transitionJobStatus(formData: FormData) {
  ...
  const { supabase } = await requireUser()
  const { error } = await supabase.rpc('transition_my_job_status', { target_job_id: jobId, new_status: status })
  if (error) fail(`/jobs/${jobId}/manage`, error.message)
