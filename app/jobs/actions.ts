...
export async function hireWorker(formData: FormData) {
  ...
  if (!applicationId || !jobId) fail('/jobs', 'Application not found.')
  const { supabase } = await requireUser()
  const { data: employmentId, error } = await (supabase.rpc as unknown as (functionName: string, args: Record<string, string>) => Promise<{ data: string | null; error: { message: string } | null }> )('hire_worker', { target_application_id: applicationId })
